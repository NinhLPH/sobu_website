...
export default function ProductReviewSection({ product }: ProductReviewSectionProps) {
    ...
    const [lookupType, setLookupType] = useState<ReviewOrderLookupType>('orderId');
    ...
    useEffect(() => {
        ...
        setFiles([]);
        setFormError(null);
    }, [product.id, fetchPublicReviews, resetVerification]);
    ...
    const handleVerifyOrder = async (event: FormEvent) => {
        event.preventDefault();
        clearSubmitState();
        setFormError(null);
        await verifyOrderForProduct(product, lookupType, orderReference);
    };
    ...
    const handleSubmitReview = async (event: FormEvent) => {
        ...
        if (!verifiedOrder || !verificationResult?.canReview) {
            setFormError('Vui lòng xác minh đơn hàng đã giao trước khi gửi đánh giá.');
            return;
        }
        ...
        <section id="product-reviews" className="mt-14 border-t border-surface-container-high pt-10">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="flex items-center gap-3 rounded-full bg-surface-container px-4 py-2 text-xs font-black text-on-surface">
                    ...
                    <span className="text-outline">({reviewsPage.totalElements} đánh giá)</span>
                </div>
            </div>
                    ...
                    {!isAuthenticated ? (
                        <div className="space-y-3">
                            ...
                            <p className="text-xs font-medium leading-relaxed text-on-surface-variant">
                                Vui lòng đăng nhập để xác minh đơn hàng đã giao và gửi đánh giá.
                            </p>
                            <Link
                                to="/login"
                        ...
                        <>
                            <form onSubmit={handleVerifyOrder} className="space-y-3">
                                <div>
                                    <div className="flex items-center gap-2 text-sm font-black text-on-surface">
                                        <ShieldCheck className="h-4 w-4 text-primary" />
                                        Xác minh đơn hàng
                                    ...
                                    <p className="mt-1 text-xs font-medium leading-relaxed text-on-surface-variant">
                                        Nhập mã đơn hàng đã giao để mở form đánh giá sản phẩm này.
                                    </p>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setLookupType('nhanhOrderId');
                                            resetVerification();
                                        }}
                                        className={`rounded-xl px-3 py-2 text-xs font-black transition-colors ${
                                            lookupType === 'nhanhOrderId'
                                                ? 'bg-primary text-on-primary'
                                                : 'bg-surface-container text-on-surface hover:bg-surface-container-high'
                                        }`}
                                    >
                                ...
                                <label className="block">
                                    <span className="mb-1 block text-[11px] font-black uppercase tracking-wider text-outline">
                                        {lookupType === 'orderId' ? 'Order ID' : 'Nhanh ID / Code'}
                                    </span>
                                    <input
                                        type="text"
                                        ...
                                        placeholder={lookupType === 'orderId' ? 'VD: 123' : 'VD: Nhanh-123456'}
                                        className="w-full rounded-xl border border-outline-variant/30 bg-surface px-4 py-3 text-sm font-bold text-on-surface outline-none transition-colors focus:border-primary"
                                    />
                                </label>
                                <button
                                    type="submit"
                                    disabled={isVerifyingOrder || !orderReference.trim()}
                                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-surface-container px-4 py-3 text-xs font-black text-on-surface transition-colors hover:bg-surface-container-high disabled:opacity-50"
                                >
                                    {isVerifyingOrder ? (
                                ...
                                {(verificationResult || verificationError) && (
                                    <div className={`flex items-start gap-2 rounded-xl p-3 text-xs font-bold ${
                                        verificationResult?.canReview
                                        ...
                                        <span>{verificationResult?.reason || verificationError}</span>
                                    </div>
                                )}
                            </form>

                            {verificationResult?.canReview && verifiedOrder && (
                                <form onSubmit={handleSubmitReview} className="space-y-4 border-t border-outline-variant/20 pt-4">
                                    <div>
