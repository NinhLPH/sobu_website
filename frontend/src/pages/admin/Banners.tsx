...
const emptyBanner = (): BannerMutationPayload => ({
    ...
    position: 'home_hero_carousel', isActive: true, startDate: '', endDate: '', deviceType: 'ALL',
});
...
const positionLabel = (position: BannerMutationPayload['position']) =>
    findBannerPositionOption(position)?.label || position;
...
export default function AdminBanners() {
    ...
    const validate = () => {
        ...
        if (!image && !form.imageUrl.trim()) return 'Hãy chọn ảnh hoặc nhập URL ảnh.';
    ...
    const submit = async (event: FormEvent) => {
        ...
        try {
            ...
            await AdminUiService.updateBanner(editing.id, formData);
            await Promise.all([load(), refreshPublic()]);
            ToastService.success('Đã cập nhật banner.');
            setModalOpen(false);
        } catch (submitError) {
    ...
    return (
        <div className="space-y-6 pt-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div><h1 className="text-2xl font-black uppercase text-on-surface">Quản lý banner</h1><p
                    className="mt-1 text-xs font-semibold text-outline">Hero, quảng cáo giữa trang và hai sidebar.</p>
                </div>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-outline-variant/30 bg-white shadow-sm">
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[900px] text-left text-xs">
                        ...
                        <tbody>
                        {loading ? <tr>
                            <td colSpan={7} className="py-16 text-center"><Loader2
                                className="mx-auto h-8 w-8 animate-spin text-primary"/></td>
                        </tr> : pageData.content.map((banner) => (
                            <tr key={banner.id} className="border-t border-outline-variant/20">
                                <td className="px-5 py-3"><img src={getPublicImageUrl(banner.imageUrl)}
                                                               alt={banner.title}
                                                               className="h-14 w-24 rounded-lg bg-surface-container object-cover"/>
                                </td>
                                <td className="max-w-xs px-5 py-3"><p
                                    className="truncate font-black text-on-surface">{banner.title}</p><p
                                    className="mt-1 truncate text-[10px] text-outline">{banner.linkUrl || 'Không có link'}</p>
                                </td>
                                <td className="px-5 py-3"><p
                                    className="font-mono font-bold">{positionLabel(banner.position)}</p></td>
                                ...
                                <td className="px-5 py-3">
                                    <div className="flex justify-center gap-2">
                                        <button type="button" onClick={() => openEdit(banner)}
                                                ...
                                                className="rounded-lg bg-primary/10 p-2 text-primary"><Edit
                        ...
                        {!loading && !pageData.content.length && <tr>
                            <td colSpan={7} className="py-16 text-center font-bold text-outline"><ImageIcon
                                className="mx-auto mb-2 h-9 w-9 opacity-30"/>Không có banner.
                            </td>
                        </tr>}
                        </tbody>
                    </table>
                ...
                {pageData.totalPages > 1 &&
                    <div className="flex items-center justify-between border-t border-outline-variant/20 px-5 py-4">
                        <span
                            className="text-xs font-bold text-outline">Trang {pageData.pageNumber}/{pageData.totalPages}</span>
                        <div className="flex gap-2">
                            <button type="button" disabled={!pageData.hasPrevious || loading}
                                    onClick={() => setPage((value) => Math.max(1, value - 1))}
                                    className="rounded-lg bg-surface-container p-2 disabled:opacity-40"><ChevronLeft
                                className="h-4 w-4"/></button>
                            <button type="button" disabled={!pageData.hasNext || loading}
                                    onClick={() => setPage((value) => value + 1)}
                                    className="rounded-lg bg-surface-container p-2 disabled:opacity-40"><ChevronRight
                                className="h-4 w-4"/></button>
                        </div>
                    </div>}
            </div>
            ...
            {modalOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
                <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-xl">
                    ...
                    <form onSubmit={submit} className="space-y-4 p-6">
                        <div><label className="mb-1 block text-xs font-black uppercase text-outline">Tiêu
                            đề</label><input aria-label="Tiêu đề" required value={form.title}
                                             onChange={(event) => setForm({...form, title: event.target.value})}
                                             className="w-full rounded-xl border border-outline-variant px-4 py-2.5 text-sm"/>
                        </div>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Vi tri co
                                dinh</label>
                                <div
                                    className="rounded-xl border border-outline-variant bg-surface-container px-4 py-2.5 font-mono text-sm font-bold">{positionLabel(form.position)}</div>
                                {findBannerPositionOption(form.position) &&
                                    <p className="mt-1 text-[10px] font-semibold text-outline">{findBannerPositionOption(form.position)?.description}</p>}
                            </div>
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Thiet
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Ảnh tải
                                lên</label><input type="file" accept="image/*"
                                                  onChange={(event) => setImage(event.target.files?.[0] || null)}
                                                  className="w-full rounded-xl border border-outline-variant px-3 py-2 text-xs"/>
                            </div>
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Hoặc URL
                                ảnh</label><input value={form.imageUrl}
                                                  onChange={(event) => setForm({...form, imageUrl: event.target.value})}
                                                  placeholder="/api/public/files/banners/..."
                                                  className="w-full rounded-xl border border-outline-variant px-4 py-2.5 text-sm"/>
                            </div>
                        </div>
                        {(previewUrl || form.imageUrl) && <div className="rounded-xl bg-surface-container p-2"><img
                            src={previewUrl || getPublicImageUrl(form.imageUrl)} alt="Xem trước banner"
                            className="h-36 w-full rounded-lg object-cover"/></div>}
                        <div><label className="mb-1 block text-xs font-black uppercase text-outline">Link
                            đích</label><input value={form.linkUrl || ''}
                                               onChange={(event) => setForm({...form, linkUrl: event.target.value})}
                                               placeholder="/products hoặc https://..."
                                               className="w-full rounded-xl border border-outline-variant px-4 py-2.5 text-sm"/>
                        </div>
                        <div className="grid gap-4 sm:grid-cols-3">
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Thứ
                                tự</label><input type="number" min={0} value={form.displayOrder}
                            ...
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Bắt đầu</label><input
                                type="datetime-local" value={form.startDate || ''}
                                onChange={(event) => setForm({...form, startDate: event.target.value})}
                                className="w-full rounded-xl border border-outline-variant px-3 py-2.5 text-sm"/></div>
                            <div><label className="mb-1 block text-xs font-black uppercase text-outline">Kết
                                thúc</label><input type="datetime-local" value={form.endDate || ''}
                                                   onChange={(event) => setForm({...form, endDate: event.target.value})}
                                                   className="w-full rounded-xl border border-outline-variant px-3 py-2.5 text-sm"/>
                            </div>
                        </div>
                        <label className="flex items-center gap-3 text-sm font-bold"><input type="checkbox"
                                                                                            checked={form.isActive}
                                                                                            onChange={(event) => setForm({
                                                                                                ...form,
                                                                                                isActive: event.target.checked
                                                                                            })}
                                                                                            className="h-4 w-4 accent-primary"/> Hoạt
                            động</label>
                        {error &&
                            <div className="rounded-xl bg-error/10 p-3 text-xs font-bold text-error">{error}</div>}
                        <div className="flex justify-end gap-3 border-t border-outline-variant/30 pt-4">
                            <button type="button" onClick={() => setModalOpen(false)}
                                    className="rounded-xl px-5 py-2.5 text-xs font-black">Hủy
                            </button>
                            <button disabled={submitting}
                                    className="flex items-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-xs font-black text-white disabled:opacity-50">{submitting &&
                                <Loader2 className="h-4 w-4 animate-spin"/>} Lưu banner
                            </button>
                        </div>
                    </form>
                </div>
