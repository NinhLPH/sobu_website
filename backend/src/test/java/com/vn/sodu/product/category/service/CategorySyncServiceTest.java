...
class CategorySyncServiceTest {
    ...
    void testBuildCategoryListUrl() {
        ...
        String url = categorySyncService.buildCategoryListUrl();
    ...
    void testBuildCategoryListRequestBody() {
        Map<String, Object> body = categorySyncService.buildCategoryListRequestBody(null);
    ...
    @Test
    @DisplayName("Should collect categories across next cursors and skip invalid items")
    void testFetchAllCategoriesMultiPage() {
        ...
        List<NhanhCategoryDTO> result = categorySyncService.fetchAllCategoriesWithFetcher(fetcher);
    ...
    @Test
    @DisplayName("Should throw when category response is null")
    void testFetchAllCategoriesNullResponseThrows() {
        RuntimeException ex = assertThrows(RuntimeException.class,
                () -> categorySyncService.fetchAllCategoriesWithFetcher(cursor -> null));

        assertTrue(ex.getMessage().toLowerCase().contains("invalid response"));
    }

    @Test
    @DisplayName("Should throw when category response code is not success")
    void testFetchAllCategoriesErrorCodeThrows() {
        RuntimeException ex = assertThrows(RuntimeException.class,
                () -> categorySyncService.fetchAllCategoriesWithFetcher(cursor -> new NhanhResponse<>(0, null, null)));
